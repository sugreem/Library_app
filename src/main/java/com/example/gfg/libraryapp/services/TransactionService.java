...
public class TransactionService {
    ...
    public String issueBook(IssueBookRequest issueBookRequest) throws Exception {
        ...
        if(student.getBooks().size() >= maxAllowedBooks){
            throw new Exception("Maximum number of books have already assigned to the student");
        }
        ...
        try {
           ...
           // transaction.setTransactionTime(new Date());   //needed when @Transactional is added
            transactionRepository.save(transaction);   //update the already created entry in line no 59
    ...
    public String returnBook(ReturnBookRequest returnBookRequest) throws Exception {
        ...
        long issueTransactionMillis = issueTransaction.getTransactionTime().getTime();
